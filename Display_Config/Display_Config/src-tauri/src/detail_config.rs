...
/// The engine's terrain batch uses 16-bit vertex indices, so the VISIBLE
/// terrain must stay under 65,536 vertices — past that, wrapped indices draw
/// the far patches as shredded/missing triangles (reported on Alpine Easy at
...
pub fn max_safe_render_distance(ground_detail: i32) -> i32 {
    match ground_detail {
        4 => 470,  // 289 verts/patch -> budget exhausts ~482 (measured 480 ok / 500 shredded)
