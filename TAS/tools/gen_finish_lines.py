"""Generate the Rust finish-line table for tas_ui from levelData.json."""
import json
...
print("pub const FINISH_POINTS: &[(&str, &[[f32; 3]])] = &[")
for lvl in sorted(d, key=lambda k: CODE.get(k, "zz")):
    if lvl not in CODE:
        continue
    pts = [i["position"] for i in d[lvl] if i["name"] == "Finish_Point"]
