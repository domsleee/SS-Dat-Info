...
impl RecordingFile {
    ...
    pub fn save_with_segments(state: &TasSharedState, path: &std::path::Path, segments: &[Segment]) -> Result<(), String> {
    ...
    pub fn load(state: &mut TasSharedState, path: &std::path::Path) -> Result<u32, String> {
        ...
        state.force_fixed_tick = meta.force_fixed_tick;
...
pub fn save_dialog_with_segments(state: &TasSharedState, segments: &[Segment], log: &mut Vec<String>) {
...
pub fn load_dialog(state: &mut TasSharedState, log: &mut Vec<String>) {
    if let Some(path) = rfd::FileDialog::new()
...
pub fn dump_diagnostics(state: &TasSharedState, log: &mut Vec<String>) {
        ...
        .save_file()
    {
        ...
        out.push_str(&format!(
            "Config: inject_mode={} fft={} force_direct={} input_source={}\n",
